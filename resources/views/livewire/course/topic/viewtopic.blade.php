                    ...
                    <div class="bg-gray-900 lg:h-screen lg:w-[24rem] overflow-hidden lg:mr-24 p-3 lg:mt-0 mt-12 lg:p-12">
                        <div class="rounded bg-white p-5">
                            <a href="{{ route('course.course_details', ['course'
                                => $this->course->key]) }}"
                                class="underline tracking-tight text-lg lg:text-xl font-semibold">
                        ...
                        <div class="space-y-2 mt-5">
                            @foreach ($this->course->topics as $topic)
                                <div
                                    ...
                                    <p class="tracking-tight text-sm font-semibold text-white">
                                        {{ $topic->title }}</p>
                                    <a href="{{ route('course.topic', ['course' => $this->course->key, 'topic' => $topic->key]) }}"
                                        class="bg-gray-800 rounded font-bold text-white text-xs px-3 py-2 rounde">
                                        Start
                                    </a>
                                </div>
                            @endforeach
            ...
            <article class="lg:z-30 lg:px-5 pt-12 lg:col-span-2" x-data="{ open: false }">
                <div class="space-y-4">
                    ...
                    <p x-show="open === false" @click="open = !open"
                        class="cursor-pointer text-sm tracking-tight

<section class="relative" 
    ...
    startTimer: function() {
        this.time_left = this.timerDuration();
        ...
        <article class="py-5 lg:py-0">
            <div class="px-5">
                ...
                <section class="lg:pt-8">
                    <section wire:target="toggleBookmark" wire:loading.remove :class="{
                        'blur': !startQuiz
                    }">
        ...
        <div class="p-4 bg-white lg:w-1/3 m-auto rounded space-y-4">
            ...
            <div class="flex flex-col gap-3 text-sm">
                <span class="flex gap-2">
                    <input id="free" type="radio" name="quiz_mode" x-model="quiz_mode" value="Free" class="text-indigo-600">
                    <label for="free">Free mode - <small class="italic">Answer Questions Freely</small></label>
                </span>
                {{--  --}}
                <span class="flex gap-2">
                    <input id="timer" type="radio" name="quiz_mode" x-model="quiz_mode" value="Timer" class="text-indigo-600">
                    <label for="timer">Timed mode - <small class="italic">Answer Questions Before Timeout</small></label>
                </span>
                {{-- timed mode options --}}
                <div 
                    x-transition.show 
                    x-cloak 
                    x-show="quiz_mode == `Timer`" 
                    class="flex gap-4 flex-wrap text-xs">
                    <button 
                        class="p-3 border  rounded-lg  "
                        x-on:click="timer_speed = 1"
                        x-bind:class="{
                    ...
                    <button 
                        ...
                        >Lightning speed (1.5x faster)</button>
                </div>
                {{-- show time --}}
                <div x-cloak x-show="quiz_mode == `Timer`" class="flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor" class="w-6 h-6 text-indigo-400/60">
            ...
            <button wire:click="toggleBookmark"
                ...
                class="w-full font-semibold px-4 p-2 tracking-wider rounded flex justify-center"
                ></button>
            @else
            <button @click="$modals.show('login-modal')"
                class="w-full font-semibold px-4 p-2 tracking-wider text-indigo-600 bg-indigo-200/30 rounded flex justify-center"
                >Sign in to bookmark quiz</button>

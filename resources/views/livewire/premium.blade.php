...
<section class="grid lg:grid-cols-2 grid-cols-1 lg:h-screen">
    <article class="bg-gray-800 lg:py-12 lg:px-28 px-5 py-5">
        ...
        <form class="mt-6" method="POST" wire:submit.prevent="store">
            ...
            <div class="mt-4">
                ...
                <x-text-input placeholder="enter first name" id="first_name" class="block mt-1 w-full" wire:model.defer="first_name"
                type="text" name="first_name" :value="old('first_name')" autocomplete="first_name"
                required />
            ...
            <div class="mt-4">
                ...
                <x-text-input placeholder="enter email address" id="email" class="block mt-1 w-full" wire:model.defer="email" type="email" name="email" :value="old('email')" autocomplete="username" required />
            ...
            <div class="flex my-8">
                <label for="terms" class="flex items-center">
                    <input id="terms" required type="checkbox" class="text-purple-500
                        focus:ring-0 rounded h-4 w-4 form-checkbox" name="terms">
                    <span class="ml-2 text-xs sm:text-sm text-white tracking-wider">{{__('Agree to our terms of use and privacy.')}}</span>
                </label>
            </div>

            <button class="mt-5 bg-purple-700 py-2 text-base font-semibold text-white
    ...
    <article class="">
        <article class="lg:py-16 lg:px-36 px-5 py-5" x-data="{ current_plan: $persist(@entangle('activePlan')) }">
            <h4 class="mb-8 tracking-wider text-gray-800 text-2xl font-bold">Choose Plan</h4>

            <section class="space-y-3">
                @foreach ( [
                            ['id' => 1, 'name' => 'Monthly', 'price' => '12/month'],
                            ['id' => 2, 'name' => 'Semester', 'price' => '24/semester'],
                            ['id' => 3, 'name' => 'Session', 'price' => '55/session']
                        ] as $delivery )
                    <div @click="current_plan = {{$delivery['id']}}"
                        :class="{
                                'border-purple-500 text-purple-700 font-medium' : current_plan === {{$delivery['id']}},
                                'border-slate-300 text-gray-600' : current_plan !== {{$delivery['id']}},
                            }"
                        ...
                        <div
                            :class="{
                                    'bg-purple-700' : current_plan === {{$delivery['id']}},
                                    'bg-slate-300' : current_plan !== {{$delivery['id']}}
                                    }"

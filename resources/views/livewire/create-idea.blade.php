        ...
        <form wire:submit.prevent="createIdea" action="#" method="POST" class="space-y-4 px-4 py-6">
            <div>
                <input wire:model.defer="title" type="text"
                    class="w-full text-sm bg-gray-100 border-none rounded
                    placeholder-gray-600 px-4 py-2"
                    placeholder="Enter discussion title here.." required>
                @error('title')
            ...
            <div>
                <textarea wire:model.defer="description" name="idea" id="idea" cols="30" rows="4"
                    ...
                    rounded border-none placeholder-gray-600 text-sm px-4 py-2"
                    placeholder="Start your discussion..." required></textarea>
                @error('description')
                    <p class="text-red text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
    ...
    @else
        <div class="my-6 text-center">
            ...
            <a wire:click.prevent="redirectToRegister" href="{{ route('premium') }}"
                class="inline-block justify-center w-1/2 h-11 text-sm tracking-wider bg-gray-200 font-semibold rounded border border-gray-200 hover:border-gray-400 transition duration-150 ease-in px-6 py-3 mt-4">
                Sign Up
            </a>
        </div>
    @endauth
